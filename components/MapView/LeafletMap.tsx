...
const StationMarkers: React.FC<{
  setHoverInfo: (
    info: { station: Station; x: number; y: number } | null
  ) => void;
}> = ({ setHoverInfo }) => {
  const map = useMap();
  // Use stationsOnMap for filtering
  const { stationsOnMap, currentStation, selectStation, play, isDarkMode } =
    useAppStore();
  ...
  const handleStationClick = useCallback(
    (station: Station) => {
      ...
      const stationIndex = stationsOnMap.findIndex(
  ...
  const activeStationMarker = useMemo(() => {
    ...
    return (
      <Marker
        ...
        icon={L.divIcon({
          className: "",
          html: '<div class="station-marker-active"></div>',
        })}
        zIndexOffset={1000}
      />
    );
    ...
    <div className="relative w-screen h-screen">
      <div className="absolute top-0 left-0 w-full h-full z-10">
        <MapContainer
          ...
          zoomDelta={0.5}
        >
          <TileLayer url={isDarkMode ? darkThemeUrl : lightThemeUrl} />
